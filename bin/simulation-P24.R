...
data$X1 <- panel.data$X
data$X2 <- panel.data$X
data$Y <- panel.data$Y
...
init.param <- function ( data ) {
  ...
  output$net <- sample(0:1, size=data$size^2, replace=TRUE, prob=c(0.95,0.05)) |>
    matrix(nrow=data$size) |>
    (`diag<-`)(0)
  output$W <- RowNormalize(output$net)
  output$theta <- rep(0,1)
  output$sigma2 <- 1
  output
}
...
sample.posterior$lambda <- function ( data, param ) {
  log.density <- function ( lambda ) LogLikelihoodAll(
    ...
    lambda = lambda,
    W = param$W,
    beta1 = param$beta[1:(length(param$beta)/2)],
    beta2 = rep(0, length(param$beta)/2),
    sigma2 = param$sigma2
  )
...
gibbs$run <- function (
  ...
  for ( iteration in start.from.iteration:(chain.length-start.from.iteration+1) ) {
    param$beta   <- c(sample.posterior$beta(data,param), rep(0, length(param$beta)/2))
